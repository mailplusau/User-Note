...
function userNote(request, response) {
	if (request.getMethod() == "GET") {
		...
		var form = nlapiCreateForm('Create User Note: <a href="' + baseURL + '/app/common/entity/custjob.nl?id=' + customer_id + '">' + recCustomer.getFieldValue('entityid') + '</a> ' + recCustomer.getFieldValue('companyname'));

		form.addField('custpage_customer_id', 'integer', 'Customer ID').setDisplayType('hidden').setDefaultValue(customer_id);
		form.addField('custpage_suitlet', 'textarea', 'Latitude').setDisplayType('hidden').setDefaultValue(script_id);
		form.addField('custpage_reason', 'textarea', 'Latitude').setDisplayType('hidden').setDefaultValue(reason);
		form.addField('custpage_deploy', 'textarea', 'Latitude').setDisplayType('hidden').setDefaultValue(deploy_id);
		form.addField('custpage_type', 'textarea', 'Latitude').setDisplayType('hidden').setDefaultValue(type);
		form.addField('custpage_cancel', 'textarea', 'Latitude').setDisplayType('hidden').setDefaultValue(cancel);
		...
		inlineHtml += '<div class="col-xs-6 notetype"><div class="input-group"><span class="input-group-addon" id="notetype_text">NOTE TYPE </span><select id="notetype" class="form-control notetype" required><option></option>';
